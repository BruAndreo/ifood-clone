...
public class ConstraintViolationImpl implements Serializable {
    ...
    @Schema(description = "Mensagem descritiva do erro possibelmente associado ao path", required = true)
    private final String mensagem;

    private ConstraintViolationImpl(ConstraintViolation<?> violation) {
        this.mensagem = violation.getMessage();
        this.atributo = Stream.of(Arrays.stream(violation.getPropertyPath().toString().split("\\.")).skip(2).collect(Collectors.joining("."))).toString();
    }

    public ConstraintViolationImpl(String atributo, String messagem) {
